...
style = Style([("top", "bg:#5b29bc fg:#ffffff bold")])
...
class Editor:
    ...
    def __init__(self):
        ...
        self.buffer1 = Buffer()
        self.text_window = Window(content=BufferControl(buffer=self.buffer1))
        self.text_scroll = ScrollablePane(self.text_window, show_scrollbar=False)
        ...
        hotkey_panel = VSplit(
            [
                VSplit(
                    [
                        Window(
                            content=FormattedTextControl(text=f"{hotkey}"),
                            width=Dimension(min=3, max=3),
                            style="fg:#00ffaa bold",
                        ),
                        Window(content="", width=Dimension(min=1, max=1)),
                        Window(
                            content=FormattedTextControl(text=f"{desc}"),
                            width=Dimension(min=1, max=24),
                            style="fg:#ffffff bold",
                        ),
                    ],
                )
                for hotkey, desc in self.hotkeys.items()
            ]
        )
        return hotkey_panel
...
def exit_(event):
    editor.pop_up_confirm("Exit from file? (y/n)", "#00ccff", yes_cb=event.app.exit)
...
def write_out_(event, filter=~is_pop_up):
    ...
    if hasattr(editor, "file_name"):
        ...
        file_name = ""
    editor.pop_up_text_field(
        "File name: ",
        "#ffccff",
        prefill=file_name,
        yes_cb=save_file,
        call_cb_with_buffer=True,
    )


@kb.add("c-a")
def select_all(event, filter=~is_pop_up):
    data = ClipboardData(editor.buffer1.text)
    editor.clipboard.set_data(data)
    editor.pop_up_text("Copied all to clipboard", "#00ffff")
...
def cli(filename: str, version: bool = False, hi_dave: bool = False) -> int:
    ...
    # try:
    if version:
        click.echo(f"You are using: npp {__VERSION__}")
        return 0
    if hi_dave:
        click.echo(
            "OMG "
            + click.style("SWARM6IX", fg="red")
            + " Bassist and "
            + click.style("SWARMED", fg="green")
            + " guitarist "
            + click.style("Dave Curtis", italic=True)
            + "!"
        )
        return 0
    if filename is None:
